...
class CameraDetection:
    ...
    def _read_usb_commands(self) -> None:
        """Read and process incoming USB commands from the Raspberry Pi."""
        try:
            if not self._usb.isconnected():
                return
            n = self._usb.any()
            if not n:
                return
            data = self._usb.read(n)
            if not data:
                return
            # Accept newline separated commands or single-byte commands
            parts = data.replace(b'\r', b'').split(b'\n')
            for part in parts:
                if not part:
                    continue
                try:
                    ...
                    # fallback for single byte commands
                    try:
                        if isinstance(part, (bytes, bytearray)) and len(part) == 1:
                            cmd = part[0]
                        else:
                            continue
                    except Exception:
    ...
    def _handle_cmd(self, cmd: int) -> None:
        """Handle parsed integer commands.
        0 -> restart
        1 -> stop (exit)
        """
        try:
            if cmd == 0:
                ...
                print("CMD received: stop (exit loop)")
                self.EXIT = True
            else:
    ...
    def check_image(self) -> None:
        ...
        center_y = int(self.img.height() / 2)
        self.img.draw_line(0, center_y, self.img.width(), center_y)

        quarter = self._getQuarter()

        dis = [x_cm, y_cm, blueX, blueY, yellowX, yellowY, quarter]
    ...
    def _getQuarter(self):
        ...
        blob_cx = self.blob.cx()
        blob_cy = self.blob.cy()

        if blob_cx > center_x and blob_cy < center_y:
            return 1
        elif blob_cx < center_x and blob_cy < center_y:
            return 2
        elif blob_cx < center_x and blob_cy > center_y:
            return 3
        elif blob_cx > center_x and blob_cy > center_y:
